...
function carregarDashboard() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const resposta = yield fetch('../api/produtos.php');
            if (!resposta.ok) {
                throw new Error(`Erro na requisição: Status ${resposta.status}`);
            }
            const produtos = yield resposta.json();
...
function atualizarCards(produtos) {
    ...
    if (produtos.length === 0) {
        if (elTotal)
            elTotal.innerText = '0';
        if (elAtivos)
            elAtivos.innerText = 'Nenhum produto registrado';
        return;
    }
...
function exibirTabela(produtos) {
    ...
    produtos.forEach((item) => {
        ...
        tr.innerHTML = `
            ...
            <td class="text-center">${(_a = item.categoria) !== null && _a !== void 0 ? _a : 'Sem categoria'}</td>
